...
struct JiraProjectController: RouteCollection {
    func boot(routes: RoutesBuilder) throws {
        let users = routes.grouped("api").grouped("jira").grouped("projects")
        users.get(use: add)
    }
    ...
    func add(req: Request) throws -> EventLoopFuture<Response> {
        let add = try req.query.decode(JiraAddUserC.self)
        return JiraAccount.find(add.jira_account.uuid, on: req.db).flatMap { account -> EventLoopFuture<Response> in
            
            guard let a = account else {return req.eventLoop.makeSucceededFuture(req.redirect(to: "web/jira/accounts", type: .temporary))}
            let uri = URI(string: AppConfig.jiraBaseURL + "issue/createmeta")
            if let c = req.webToJiraRest(url: uri, token: a.basicAuth, method: .GET, body: nil) {
